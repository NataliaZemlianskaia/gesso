...
We generate a sparse genotype matrix `G` by setting the genotype prevalence parameter `pG` to 0.1 and measure how long does it take to fit the model.
...
We see that the sparse option enables (`r round(as.numeric(time_non_sparse) / as.numeric(time_sparse), 2)` times) more efficient fitting. Higher speed-up can be observed for large sparse matrices.
...
We use the same large dataset (`p` = 10,000) that we generated for the sparse matrices example to demonstrate how our screening rules work.
