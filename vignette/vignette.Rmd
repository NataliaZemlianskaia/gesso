...
The package allows adding unpenalized covariates to the model.
...
There is an option to use a custom grid in `gesso.cv()` function by specifying `grid` parameter. 
...
sum(!tune_model$fit$has_converged)
```
...
grid = 10^seq(-5, log10(1), length.out = 20)
...
We see that the sparse option enables more efficient fitting, however, the speed-up works only for substantially sparse matrices.
